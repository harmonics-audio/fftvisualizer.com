...
`local` mode. See [Data modes](/guide/data-modes) for the API of each.
...
(WASM) *and* in the [backend reference servers](/guide/websocket-protocol#reference-servers),
...
- **`bands`** — how many bars the visualizer actually *displays*. It aggregates the incoming
...
external hardware? The `frame` event ([Core](/core/api#events), [Vue](/vue/props#the-frame-event),
[React](/react/props#onframe)) hands you exactly what the shader draws, every frame.
::
